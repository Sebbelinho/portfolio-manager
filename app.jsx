...
let _abortCtrl = null;
function delay(ms) {
  if (_abortCtrl?.signal?.aborted) return Promise.reject(new DOMException("Aborted", "AbortError"));
  if (ms < 2000) {
    return new Promise((resolve, reject) => {
      const t = setTimeout(resolve, ms);
      _abortCtrl?.signal?.addEventListener("abort", () => { clearTimeout(t); reject(new DOMException("Aborted", "AbortError")); }, { once: true });
    });
  }
...
async function fetchFredSeries(seriesId, fredKey, limit = 2) {
  ...
  const r = await fetch(url, { signal: _abortCtrl?.signal });
...
async function fetchMarketIndicators() {
  ...
  try {
    ...
    for (const [key, symbol] of Object.entries(MARKET_TICKERS)) {
      try {
        const r = await fetch(`https://finnhub.io/api/v1/quote?symbol=${symbol}&token=${token}`, { signal: _abortCtrl?.signal });
  ...
  for (const ticker of tickers) {
    ...
    try {
      ...
      const [quoteRes, metricsRes, recoRes, earningsRes] = await Promise.all([
        fetch(`https://finnhub.io/api/v1/quote?symbol=${ticker}&token=${token}`, { signal: sig }),
        fetch(`https://finnhub.io/api/v1/stock/metric?symbol=${ticker}&metric=all&token=${token}`, { signal: sig }),
        fetch(`https://finnhub.io/api/v1/stock/recommendation?symbol=${ticker}&token=${token}`, { signal: sig }),
        fetch(`https://finnhub.io/api/v1/stock/earnings?symbol=${ticker}&token=${token}`, { signal: sig }),
      ]);
...
async function verify52WeekHighs(stockData) {
  // Nur Ticker mit unplausiblen 52wH-Werten verifizieren:
  // - yearHigh == null/undefined
  // - yearHigh <= aktueller Kurs (sollte immer >= sein)
  // - yearHigh == aktueller Kurs (verdächtig identisch)
  const suspect = Object.entries(stockData).filter(([, d]) => {
    if (d.yearHigh == null || d.price == null) return true;
    if (d.yearHigh <= d.price) return true;
    return false;
  });
  if (suspect.length === 0) {
    const ts = new Date().toLocaleTimeString("de-DE", { hour: "2-digit", minute: "2-digit", second: "2-digit" });
    debugPush({ ts, label: `52wH plausibel: alle ${Object.keys(stockData).length} Ticker ok — kein Web-Check nötig`, status: "ok", search: false, tokens: 0 });
    return stockData;
  }
  const suspectTickers = suspect.map(([t]) => t);
  const tickerList = suspect.map(([t, d]) => `${t}: Kurs=$${d.price}, Finnhub52wH=$${d.yearHigh ?? "n/a"}`).join(", ");
  const ts = new Date().toLocaleTimeString("de-DE", { hour: "2-digit", minute: "2-digit", second: "2-digit" });
  debugPush({ ts, label: `52wH Verifizierung: ${suspectTickers.length} verdächtig (${suspectTickers.join(", ")})`, status: "pending", search: true, tokens: 500 });
  const dbIdx = _debugLog.length - 1;
  try {
    const raw = await callAPI(
      `Look up the current 52-week high price (in USD) for each of these stocks: ${suspectTickers.join(", ")}.

These stocks have suspicious Finnhub values (52-week high equals or is below current price): ${tickerList}

Respond ONLY with raw JSON, no backticks:
{"highs":{"TICKER":123.45}}
Include ALL listed tickers. Use the actual 52-week high from current market data.`,
    ...
    if (j && j.highs) {
      ...
      _debugLog[dbIdx] = { ..._debugLog[dbIdx], status: "ok", code: 200, label: corrections.length > 0
        ...
        : `52wH verifiziert: ${suspectTickers.length} Ticker geprüft, keine Korrektur nötig` };
      _debugListeners.forEach(fn => fn([..._debugLog]));
      return updated;
    }
...
async function fetchInsiderData(tickers) {
  ...
  for (const ticker of tickers) {
    ...
    const dbIdx = _debugLog.length - 1;
    try {
      const r = await fetch(`https://finnhub.io/api/v1/stock/insider-transactions?symbol=${ticker}&token=${token}`, { signal: _abortCtrl?.signal });
      const data = await r.json();
...
async function fetchEarningsCalendar(token, portfolioTickers) {
  ...
  try {
    const results = await Promise.all(allTickers.map(async (sym) => {
      try {
        const r = await fetch(`https://finnhub.io/api/v1/calendar/earnings?symbol=${sym}&from=${from}&to=${to}&token=${token}`, { signal: _abortCtrl?.signal });
        const data = await r.json();
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      if (e.name === "AbortError") {
        _debugLog[idx] = { ..._debugLog[idx], status: "error", code: 0, detail: "Abgebrochen" };
        _debugListeners.forEach(fn => fn([..._debugLog]));
        throw e;
      }
...
function App() {
  ...
  const [bucketColors, setBucketColors] = useState({ left: "#22c55e", right: "#ef4444" });
  const [editingBucket, setEditingBucket] = useState(null);
  const [bucketNameDraft, setBucketNameDraft] = useState("");
  const [colorPickerBucket, setColorPickerBucket] = useState(null);
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      if (saved.bucketColors) setBucketColors({ left: saved.bucketColors.left || "#22c55e", right: saved.bucketColors.right || "#ef4444" });
  ...
  const addStock = useCallback(() => {
    ...
    setStocks(prev => {
      ...
      saveData({ stocks: updated, capex, tsmc, dram, nvidia, positions, insider, analysis, timing, finnhubData, insiderData, lastRun: lastRun?.toISOString(), logs, dcaPlan, dcaBudget, dcaMonths, dcaExtra, bucketNames, bucketColors });
      return updated;
    });
  ...
  const removeStock = useCallback((ticker) => {
    setStocks(prev => {
      ...
      setPositions(prevPos => {
        ...
        saveData({ stocks: updated, capex, tsmc, dram, nvidia, positions: newPos, insider, analysis, timing, finnhubData, lastRun: lastRun?.toISOString(), logs, bucketNames, bucketColors });
        return newPos;
      });
      return updated;
    });
...
{"summary":"3-4 Sätze Gesamteinschätzung deutsch","impact":"positive|negative|neutral","winners":[{"ticker":"XXX","reason":"1 Satz deutsch"}],"losers":[{"ticker":"XXX","reason":"1 Satz deutsch"}],"guidance_changes":"2-3 Sätze zu relevanten Guidance-Änderungen deutsch"}`,
      ...
      setLogs(prevLogs => {
        saveData({ stocks, capex: lCapex, tsmc: lTsmc, dram: lDram, nvidia: lNvidia, insider: lInsider, analysis: ana, timing: tim, finnhubData: fmpData, insiderData: lInsiderData, macro: lMacro, marketIndicators: lMarket, lastRun: now.toISOString(), logs: prevLogs, dcaPlan, dcaBudget, dcaMonths, dcaExtra, capexImpact: lCapexImpact, geopolitik: lGeopolitik, bucketNames, bucketColors });
        return prevLogs;
      });
    } catch (e) {
      if (e.name === "AbortError") {
        const ts = new Date().toLocaleTimeString("de-DE", { hour: "2-digit", minute: "2-digit", second: "2-digit" });
        debugPush({ ts, label: "⛔ Analyse abgebrochen (manuell)", status: "error", tokens: 0, code: 0 });
        addLog("⛔ Abgebrochen.");
      } else {
  ...
  const commitBucketColor = (which, color) => {
    const next = { ...bucketColors, [which]: color };
    setBucketColors(next);
    const existing = loadData() || {};
    saveData({ ...existing, bucketColors: next });
  };
  ...
  return React.createElement("div", { style: { minHeight: "100vh", background: "#0a0e1a", color: "#e2e8f0", fontFamily: "'DM Sans',system-ui,sans-serif", padding: "16px 10px", boxSizing: "border-box", maxWidth: "100vw", overflow: "hidden" } },
    ...
    React.createElement("div", { style: { maxWidth: 860, margin: "0 auto", overflow: "hidden" } },
        ...
        React.createElement(React.Fragment, null,
          ...
          React.createElement("div", { className: "bucket-row", style: { display: "flex", gap: 8, alignItems: "stretch" } },
            ["left", "neutral", "right"].map(b => {
              ...
              const accent = isNeutral ? "#475569" : bucketColors[b];
              const tileBg = isNeutral ? "#1e293b" : `${accent}15`;
              return React.createElement("div", { key: b, style: { flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 6 } },
                ...
                React.createElement("div", { className: "bucket-box", style: { background: "#111827", borderRadius: 12, border: `1px solid ${accent}44`, padding: 10, minHeight: 200, display: "flex", flexDirection: "column", gap: 6, position: "relative" } },
                  React.createElement("div", { style: { display: "flex", alignItems: "center", justifyContent: "center", padding: "4px 4px 4px 8px", background: `${accent}22`, borderRadius: 8, marginBottom: 4, minHeight: 24, gap: 6 } },
                    React.createElement("div", { style: { flex: 1, display: "flex", alignItems: "center", justifyContent: "center", minWidth: 0 } },
                      isNeutral
                    ...
                    !isNeutral && React.createElement("button", {
                      onClick: () => setColorPickerBucket(colorPickerBucket === b ? null : b),
                      title: "Farbe ändern",
                      style: { width: 16, height: 16, borderRadius: "50%", background: accent, border: "2px solid #0f172a", boxShadow: `0 0 0 1px ${accent}`, cursor: "pointer", padding: 0, flexShrink: 0 }
                    })
                  ),
                  !isNeutral && colorPickerBucket === b && React.createElement("div", {
                    ...
                    React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 6, marginBottom: 8 } },
                      ["#22c55e", "#eab308", "#f97316", "#ef4444", "#ec4899", "#a78bfa", "#6366f1", "#22d3ee"].map(c =>
                        React.createElement("button", {
                          key: c,
                          onClick: () => { commitBucketColor(b, c); setColorPickerBucket(null); },
                          title: c,
                          style: { width: 24, height: 24, borderRadius: "50%", background: c, border: bucketColors[b] === c ? "2px solid #fff" : "2px solid transparent", cursor: "pointer", padding: 0, margin: "0 auto", display: "block" }
                        })
                      )
                    ),
                    React.createElement("label", { style: { display: "flex", alignItems: "center", gap: 6, fontSize: 10, color: "#94a3b8", cursor: "pointer" } },
                      React.createElement("input", {
                        type: "color",
                        value: bucketColors[b],
                        onChange: e => commitBucketColor(b, e.target.value),
                        style: { width: 24, height: 24, border: "none", background: "none", cursor: "pointer", padding: 0 }
                      }),
                      "Eigene Farbe"
                    ),
                    React.createElement("button", {
                      onClick: () => setColorPickerBucket(null),
                      style: { marginTop: 6, width: "100%", padding: "4px 8px", background: "#1e293b", border: "1px solid #334155", borderRadius: 6, color: "#94a3b8", fontSize: 10, cursor: "pointer", fontFamily: "inherit" }
                    }, "Schließen")
                  ),
                  tiles.length === 0
      ...
      tab === "timing" && React.createElement(React.Fragment, null,
        ...
        React.createElement("button", { onClick: () => { if (busyTiming) { _abortCtrl?.abort(); return; } if (checkKeys()) runTiming(); }, disabled: busy, style: {
